...
int main(){
  ...
  char * autognuplot2[] = {"set title \"Generating random int numbers\"", "plot 'randomInt.temp'"}; // char array to execute gnuplot commands to plot second plot with the data of the random ints.
  ...
  /*
  if (f == NULL || f2 == NULL) { // exit the program if an error occured while creating the file
        printf("Cannot create the file\n"); // message to info
        exit(1);
  }

  if (plot == NULL || plot2 == NULL) { // exit the program if an error occured while creating the plot
