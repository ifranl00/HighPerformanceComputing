...
int main()
{
    ...
    srand(time(NULL));
    int a=0,i;
    omp_set_num_threads(4);//the number of threads is determined to 4 (0,1,2,3)
...
#pragma omp parallel for//this directive allows to divide the loop interations between threads
    ...
    //If there is not additional clauses as schedule, the directive will partition the iterations as equally as possible between the all the threads.
